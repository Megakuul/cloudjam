// Code generated by go generate; DO NOT EDIT.

// Package cases holds the cloud control resources of the aws cases service.
package cases

import "encoding/json"

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type CaseRule struct {
	CaseRuleArn      *string         `json:"CaseRuleArn,omitempty"`
	CaseRuleId       *string         `json:"CaseRuleId,omitempty"`
	CreatedTime      *string         `json:"CreatedTime,omitempty"`
	Description      *string         `json:"Description,omitempty"`
	DomainId         *string         `json:"DomainId,omitempty"`
	LastModifiedTime *string         `json:"LastModifiedTime,omitempty"`
	Name             *string         `json:"Name,omitempty"`
	Rule             json.RawMessage `json:"Rule,omitempty"`
	Tags             []Tag           `json:"Tags,omitempty"`
}

func (CaseRule) CloudJamType() string { return "AWS::Cases::CaseRule" }

type DomainTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Domain struct {
	CreatedTime  *string       `json:"CreatedTime,omitempty"`
	DomainArn    *string       `json:"DomainArn,omitempty"`
	DomainId     *string       `json:"DomainId,omitempty"`
	DomainStatus *DomainStatus `json:"DomainStatus,omitempty"`
	Name         *string       `json:"Name,omitempty"`
	Tags         []DomainTag   `json:"Tags,omitempty"`
}

func (Domain) CloudJamType() string { return "AWS::Cases::Domain" }

type TextAttributes struct {
	IsMultiline *bool `json:"IsMultiline,omitempty"`
}

type FieldAttributes struct {
	Text *TextAttributes `json:"Text,omitempty"`
}

type FieldTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Field struct {
	Attributes       *FieldAttributes `json:"Attributes,omitempty"`
	CreatedTime      *string          `json:"CreatedTime,omitempty"`
	Description      *string          `json:"Description,omitempty"`
	DomainId         *string          `json:"DomainId,omitempty"`
	FieldArn         *string          `json:"FieldArn,omitempty"`
	FieldId          *string          `json:"FieldId,omitempty"`
	LastModifiedTime *string          `json:"LastModifiedTime,omitempty"`
	Name             *string          `json:"Name,omitempty"`
	Namespace        *FieldNamespace  `json:"Namespace,omitempty"`
	Tags             []FieldTag       `json:"Tags,omitempty"`
	Type             *FieldType       `json:"Type,omitempty"`
}

func (Field) CloudJamType() string { return "AWS::Cases::Field" }

type LayoutTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Layout struct {
	Content          json.RawMessage `json:"Content,omitempty"`
	CreatedTime      *string         `json:"CreatedTime,omitempty"`
	DomainId         *string         `json:"DomainId,omitempty"`
	LastModifiedTime *string         `json:"LastModifiedTime,omitempty"`
	LayoutArn        *string         `json:"LayoutArn,omitempty"`
	LayoutId         *string         `json:"LayoutId,omitempty"`
	Name             *string         `json:"Name,omitempty"`
	Tags             []LayoutTag     `json:"Tags,omitempty"`
}

func (Layout) CloudJamType() string { return "AWS::Cases::Layout" }

type LayoutConfiguration struct {
	DefaultLayout *string `json:"DefaultLayout,omitempty"`
}

type RequiredField struct {
	FieldId *string `json:"FieldId,omitempty"`
}

type TemplateRule struct {
	CaseRuleId *string `json:"CaseRuleId,omitempty"`
	FieldId    *string `json:"FieldId,omitempty"`
}

type TemplateTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Template struct {
	CreatedTime         *string              `json:"CreatedTime,omitempty"`
	Description         *string              `json:"Description,omitempty"`
	DomainId            *string              `json:"DomainId,omitempty"`
	LastModifiedTime    *string              `json:"LastModifiedTime,omitempty"`
	LayoutConfiguration *LayoutConfiguration `json:"LayoutConfiguration,omitempty"`
	Name                *string              `json:"Name,omitempty"`
	RequiredFields      []RequiredField      `json:"RequiredFields,omitempty"`
	Rules               []TemplateRule       `json:"Rules,omitempty"`
	Status              *TemplateStatus      `json:"Status,omitempty"`
	Tags                []TemplateTag        `json:"Tags,omitempty"`
	TemplateArn         *string              `json:"TemplateArn,omitempty"`
	TemplateId          *string              `json:"TemplateId,omitempty"`
}

func (Template) CloudJamType() string { return "AWS::Cases::Template" }

type DomainStatus string

const (
	DomainStatusActive             DomainStatus = "Active"
	DomainStatusCreationInProgress DomainStatus = "CreationInProgress"
	DomainStatusCreationFailed     DomainStatus = "CreationFailed"
)

type FieldNamespace string

const (
	FieldNamespaceSystem FieldNamespace = "System"
	FieldNamespaceCustom FieldNamespace = "Custom"
)

type FieldType string

const (
	FieldTypeText         FieldType = "Text"
	FieldTypeNumber       FieldType = "Number"
	FieldTypeBoolean      FieldType = "Boolean"
	FieldTypeDateTime     FieldType = "DateTime"
	FieldTypeSingleSelect FieldType = "SingleSelect"
	FieldTypeUrl          FieldType = "Url"
	FieldTypeUser         FieldType = "User"
)

type TemplateStatus string

const (
	TemplateStatusActive   TemplateStatus = "Active"
	TemplateStatusInactive TemplateStatus = "Inactive"
)
