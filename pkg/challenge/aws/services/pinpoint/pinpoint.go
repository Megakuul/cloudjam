// Code generated by go generate; DO NOT EDIT.

// Package pinpoint holds the cloud control resources of the aws pinpoint service.
package pinpoint

type ADMChannel struct {
	ApplicationId *string `json:"ApplicationId,omitempty"`
	ClientId      *string `json:"ClientId,omitempty"`
	ClientSecret  *string `json:"ClientSecret,omitempty"`
	Enabled       *bool   `json:"Enabled,omitempty"`
	Id            *string `json:"Id,omitempty"`
}

func (ADMChannel) CloudJamType() string { return "AWS::Pinpoint::ADMChannel" }

type APNSChannel struct {
	ApplicationId               *string `json:"ApplicationId,omitempty"`
	BundleId                    *string `json:"BundleId,omitempty"`
	Certificate                 *string `json:"Certificate,omitempty"`
	DefaultAuthenticationMethod *string `json:"DefaultAuthenticationMethod,omitempty"`
	Enabled                     *bool   `json:"Enabled,omitempty"`
	Id                          *string `json:"Id,omitempty"`
	PrivateKey                  *string `json:"PrivateKey,omitempty"`
	TeamId                      *string `json:"TeamId,omitempty"`
	TokenKey                    *string `json:"TokenKey,omitempty"`
	TokenKeyId                  *string `json:"TokenKeyId,omitempty"`
}

func (APNSChannel) CloudJamType() string { return "AWS::Pinpoint::APNSChannel" }

type APNSSandboxChannel struct {
	ApplicationId               *string `json:"ApplicationId,omitempty"`
	BundleId                    *string `json:"BundleId,omitempty"`
	Certificate                 *string `json:"Certificate,omitempty"`
	DefaultAuthenticationMethod *string `json:"DefaultAuthenticationMethod,omitempty"`
	Enabled                     *bool   `json:"Enabled,omitempty"`
	Id                          *string `json:"Id,omitempty"`
	PrivateKey                  *string `json:"PrivateKey,omitempty"`
	TeamId                      *string `json:"TeamId,omitempty"`
	TokenKey                    *string `json:"TokenKey,omitempty"`
	TokenKeyId                  *string `json:"TokenKeyId,omitempty"`
}

func (APNSSandboxChannel) CloudJamType() string { return "AWS::Pinpoint::APNSSandboxChannel" }

type APNSVoipChannel struct {
	ApplicationId               *string `json:"ApplicationId,omitempty"`
	BundleId                    *string `json:"BundleId,omitempty"`
	Certificate                 *string `json:"Certificate,omitempty"`
	DefaultAuthenticationMethod *string `json:"DefaultAuthenticationMethod,omitempty"`
	Enabled                     *bool   `json:"Enabled,omitempty"`
	Id                          *string `json:"Id,omitempty"`
	PrivateKey                  *string `json:"PrivateKey,omitempty"`
	TeamId                      *string `json:"TeamId,omitempty"`
	TokenKey                    *string `json:"TokenKey,omitempty"`
	TokenKeyId                  *string `json:"TokenKeyId,omitempty"`
}

func (APNSVoipChannel) CloudJamType() string { return "AWS::Pinpoint::APNSVoipChannel" }

type APNSVoipSandboxChannel struct {
	ApplicationId               *string `json:"ApplicationId,omitempty"`
	BundleId                    *string `json:"BundleId,omitempty"`
	Certificate                 *string `json:"Certificate,omitempty"`
	DefaultAuthenticationMethod *string `json:"DefaultAuthenticationMethod,omitempty"`
	Enabled                     *bool   `json:"Enabled,omitempty"`
	Id                          *string `json:"Id,omitempty"`
	PrivateKey                  *string `json:"PrivateKey,omitempty"`
	TeamId                      *string `json:"TeamId,omitempty"`
	TokenKey                    *string `json:"TokenKey,omitempty"`
	TokenKeyId                  *string `json:"TokenKeyId,omitempty"`
}

func (APNSVoipSandboxChannel) CloudJamType() string { return "AWS::Pinpoint::APNSVoipSandboxChannel" }

type App struct {
	Arn  *string        `json:"Arn,omitempty"`
	Id   *string        `json:"Id,omitempty"`
	Name *string        `json:"Name,omitempty"`
	Tags map[string]any `json:"Tags,omitempty"`
}

func (App) CloudJamType() string { return "AWS::Pinpoint::App" }

type CampaignHook struct {
	LambdaFunctionName *string `json:"LambdaFunctionName,omitempty"`
	Mode               *string `json:"Mode,omitempty"`
	WebUrl             *string `json:"WebUrl,omitempty"`
}

type Limits struct {
	Daily             *int `json:"Daily,omitempty"`
	MaximumDuration   *int `json:"MaximumDuration,omitempty"`
	MessagesPerSecond *int `json:"MessagesPerSecond,omitempty"`
	Total             *int `json:"Total,omitempty"`
}

type QuietTime struct {
	End   *string `json:"End,omitempty"`
	Start *string `json:"Start,omitempty"`
}

type ApplicationSettings struct {
	ApplicationId            *string       `json:"ApplicationId,omitempty"`
	CampaignHook             *CampaignHook `json:"CampaignHook,omitempty"`
	CloudWatchMetricsEnabled *bool         `json:"CloudWatchMetricsEnabled,omitempty"`
	Id                       *string       `json:"Id,omitempty"`
	Limits                   *Limits       `json:"Limits,omitempty"`
	QuietTime                *QuietTime    `json:"QuietTime,omitempty"`
}

func (ApplicationSettings) CloudJamType() string { return "AWS::Pinpoint::ApplicationSettings" }

type BaiduChannel struct {
	ApiKey        *string `json:"ApiKey,omitempty"`
	ApplicationId *string `json:"ApplicationId,omitempty"`
	Enabled       *bool   `json:"Enabled,omitempty"`
	Id            *string `json:"Id,omitempty"`
	SecretKey     *string `json:"SecretKey,omitempty"`
}

func (BaiduChannel) CloudJamType() string { return "AWS::Pinpoint::BaiduChannel" }

type CustomDeliveryConfiguration struct {
	DeliveryUri   *string  `json:"DeliveryUri,omitempty"`
	EndpointTypes []string `json:"EndpointTypes,omitempty"`
}

type Message struct {
	Action            *string `json:"Action,omitempty"`
	Body              *string `json:"Body,omitempty"`
	ImageIconUrl      *string `json:"ImageIconUrl,omitempty"`
	ImageSmallIconUrl *string `json:"ImageSmallIconUrl,omitempty"`
	ImageUrl          *string `json:"ImageUrl,omitempty"`
	JsonBody          *string `json:"JsonBody,omitempty"`
	MediaUrl          *string `json:"MediaUrl,omitempty"`
	RawContent        *string `json:"RawContent,omitempty"`
	SilentPush        *bool   `json:"SilentPush,omitempty"`
	TimeToLive        *int    `json:"TimeToLive,omitempty"`
	Title             *string `json:"Title,omitempty"`
	Url               *string `json:"Url,omitempty"`
}

type CampaignCustomMessage struct {
	Data *string `json:"Data,omitempty"`
}

type CampaignEmailMessage struct {
	Body        *string `json:"Body,omitempty"`
	FromAddress *string `json:"FromAddress,omitempty"`
	HtmlBody    *string `json:"HtmlBody,omitempty"`
	Title       *string `json:"Title,omitempty"`
}

type InAppMessageBodyConfig struct {
	Alignment *string `json:"Alignment,omitempty"`
	Body      *string `json:"Body,omitempty"`
	TextColor *string `json:"TextColor,omitempty"`
}

type InAppMessageHeaderConfig struct {
	Alignment *string `json:"Alignment,omitempty"`
	Header    *string `json:"Header,omitempty"`
	TextColor *string `json:"TextColor,omitempty"`
}

type OverrideButtonConfiguration struct {
	ButtonAction *string `json:"ButtonAction,omitempty"`
	Link         *string `json:"Link,omitempty"`
}

type DefaultButtonConfiguration struct {
	BackgroundColor *string `json:"BackgroundColor,omitempty"`
	BorderRadius    *int    `json:"BorderRadius,omitempty"`
	ButtonAction    *string `json:"ButtonAction,omitempty"`
	Link            *string `json:"Link,omitempty"`
	Text            *string `json:"Text,omitempty"`
	TextColor       *string `json:"TextColor,omitempty"`
}

type InAppMessageButton struct {
	Android       *OverrideButtonConfiguration `json:"Android,omitempty"`
	DefaultConfig *DefaultButtonConfiguration  `json:"DefaultConfig,omitempty"`
	IOS           *OverrideButtonConfiguration `json:"IOS,omitempty"`
	Web           *OverrideButtonConfiguration `json:"Web,omitempty"`
}

type InAppMessageContent struct {
	BackgroundColor *string                   `json:"BackgroundColor,omitempty"`
	BodyConfig      *InAppMessageBodyConfig   `json:"BodyConfig,omitempty"`
	HeaderConfig    *InAppMessageHeaderConfig `json:"HeaderConfig,omitempty"`
	ImageUrl        *string                   `json:"ImageUrl,omitempty"`
	PrimaryBtn      *InAppMessageButton       `json:"PrimaryBtn,omitempty"`
	SecondaryBtn    *InAppMessageButton       `json:"SecondaryBtn,omitempty"`
}

type CampaignInAppMessage struct {
	Content      []InAppMessageContent `json:"Content,omitempty"`
	CustomConfig map[string]any        `json:"CustomConfig,omitempty"`
	Layout       *string               `json:"Layout,omitempty"`
}

type CampaignSmsMessage struct {
	Body              *string `json:"Body,omitempty"`
	EntityId          *string `json:"EntityId,omitempty"`
	MessageType       *string `json:"MessageType,omitempty"`
	OriginationNumber *string `json:"OriginationNumber,omitempty"`
	SenderId          *string `json:"SenderId,omitempty"`
	TemplateId        *string `json:"TemplateId,omitempty"`
}

type MessageConfiguration struct {
	ADMMessage     *Message               `json:"ADMMessage,omitempty"`
	APNSMessage    *Message               `json:"APNSMessage,omitempty"`
	BaiduMessage   *Message               `json:"BaiduMessage,omitempty"`
	CustomMessage  *CampaignCustomMessage `json:"CustomMessage,omitempty"`
	DefaultMessage *Message               `json:"DefaultMessage,omitempty"`
	EmailMessage   *CampaignEmailMessage  `json:"EmailMessage,omitempty"`
	GCMMessage     *Message               `json:"GCMMessage,omitempty"`
	InAppMessage   *CampaignInAppMessage  `json:"InAppMessage,omitempty"`
	SMSMessage     *CampaignSmsMessage    `json:"SMSMessage,omitempty"`
}

type SetDimension struct {
	DimensionType *string  `json:"DimensionType,omitempty"`
	Values        []string `json:"Values,omitempty"`
}

type EventDimensions struct {
	Attributes map[string]any `json:"Attributes,omitempty"`
	EventType  *SetDimension  `json:"EventType,omitempty"`
	Metrics    map[string]any `json:"Metrics,omitempty"`
}

type CampaignEventFilter struct {
	Dimensions *EventDimensions `json:"Dimensions,omitempty"`
	FilterType *string          `json:"FilterType,omitempty"`
}

type CampaignQuietTime struct {
	End   *string `json:"End,omitempty"`
	Start *string `json:"Start,omitempty"`
}

type Schedule struct {
	EndTime     *string              `json:"EndTime,omitempty"`
	EventFilter *CampaignEventFilter `json:"EventFilter,omitempty"`
	Frequency   *string              `json:"Frequency,omitempty"`
	IsLocalTime *bool                `json:"IsLocalTime,omitempty"`
	QuietTime   *CampaignQuietTime   `json:"QuietTime,omitempty"`
	StartTime   *string              `json:"StartTime,omitempty"`
	TimeZone    *string              `json:"TimeZone,omitempty"`
}

type Template struct {
	Name    *string `json:"Name,omitempty"`
	Version *string `json:"Version,omitempty"`
}

type TemplateConfiguration struct {
	EmailTemplate *Template `json:"EmailTemplate,omitempty"`
	PushTemplate  *Template `json:"PushTemplate,omitempty"`
	SMSTemplate   *Template `json:"SMSTemplate,omitempty"`
	VoiceTemplate *Template `json:"VoiceTemplate,omitempty"`
}

type WriteTreatmentResource struct {
	CustomDeliveryConfiguration *CustomDeliveryConfiguration `json:"CustomDeliveryConfiguration,omitempty"`
	MessageConfiguration        *MessageConfiguration        `json:"MessageConfiguration,omitempty"`
	Schedule                    *Schedule                    `json:"Schedule,omitempty"`
	SizePercent                 *int                         `json:"SizePercent,omitempty"`
	TemplateConfiguration       *TemplateConfiguration       `json:"TemplateConfiguration,omitempty"`
	TreatmentDescription        *string                      `json:"TreatmentDescription,omitempty"`
	TreatmentName               *string                      `json:"TreatmentName,omitempty"`
}

type CampaignCampaignHook struct {
	LambdaFunctionName *string `json:"LambdaFunctionName,omitempty"`
	Mode               *string `json:"Mode,omitempty"`
	WebUrl             *string `json:"WebUrl,omitempty"`
}

type CampaignLimits struct {
	Daily             *int `json:"Daily,omitempty"`
	MaximumDuration   *int `json:"MaximumDuration,omitempty"`
	MessagesPerSecond *int `json:"MessagesPerSecond,omitempty"`
	Session           *int `json:"Session,omitempty"`
	Total             *int `json:"Total,omitempty"`
}

type Campaign struct {
	AdditionalTreatments        []WriteTreatmentResource     `json:"AdditionalTreatments,omitempty"`
	ApplicationId               *string                      `json:"ApplicationId,omitempty"`
	Arn                         *string                      `json:"Arn,omitempty"`
	CampaignHook                *CampaignCampaignHook        `json:"CampaignHook,omitempty"`
	CampaignId                  *string                      `json:"CampaignId,omitempty"`
	CustomDeliveryConfiguration *CustomDeliveryConfiguration `json:"CustomDeliveryConfiguration,omitempty"`
	Description                 *string                      `json:"Description,omitempty"`
	HoldoutPercent              *int                         `json:"HoldoutPercent,omitempty"`
	IsPaused                    *bool                        `json:"IsPaused,omitempty"`
	Limits                      *CampaignLimits              `json:"Limits,omitempty"`
	MessageConfiguration        *MessageConfiguration        `json:"MessageConfiguration,omitempty"`
	Name                        *string                      `json:"Name,omitempty"`
	Priority                    *int                         `json:"Priority,omitempty"`
	Schedule                    *Schedule                    `json:"Schedule,omitempty"`
	SegmentId                   *string                      `json:"SegmentId,omitempty"`
	SegmentVersion              *int                         `json:"SegmentVersion,omitempty"`
	Tags                        map[string]any               `json:"Tags,omitempty"`
	TemplateConfiguration       *TemplateConfiguration       `json:"TemplateConfiguration,omitempty"`
	TreatmentDescription        *string                      `json:"TreatmentDescription,omitempty"`
	TreatmentName               *string                      `json:"TreatmentName,omitempty"`
}

func (Campaign) CloudJamType() string { return "AWS::Pinpoint::Campaign" }

type EmailChannel struct {
	ApplicationId               *string `json:"ApplicationId,omitempty"`
	ConfigurationSet            *string `json:"ConfigurationSet,omitempty"`
	Enabled                     *bool   `json:"Enabled,omitempty"`
	FromAddress                 *string `json:"FromAddress,omitempty"`
	Id                          *string `json:"Id,omitempty"`
	Identity                    *string `json:"Identity,omitempty"`
	OrchestrationSendingRoleArn *string `json:"OrchestrationSendingRoleArn,omitempty"`
	RoleArn                     *string `json:"RoleArn,omitempty"`
}

func (EmailChannel) CloudJamType() string { return "AWS::Pinpoint::EmailChannel" }

type EmailTemplate struct {
	Arn                  *string        `json:"Arn,omitempty"`
	DefaultSubstitutions *string        `json:"DefaultSubstitutions,omitempty"`
	HtmlPart             *string        `json:"HtmlPart,omitempty"`
	Id                   *string        `json:"Id,omitempty"`
	Subject              *string        `json:"Subject,omitempty"`
	Tags                 map[string]any `json:"Tags,omitempty"`
	TemplateDescription  *string        `json:"TemplateDescription,omitempty"`
	TemplateName         *string        `json:"TemplateName,omitempty"`
	TextPart             *string        `json:"TextPart,omitempty"`
}

func (EmailTemplate) CloudJamType() string { return "AWS::Pinpoint::EmailTemplate" }

type EventStream struct {
	ApplicationId        *string `json:"ApplicationId,omitempty"`
	DestinationStreamArn *string `json:"DestinationStreamArn,omitempty"`
	Id                   *string `json:"Id,omitempty"`
	RoleArn              *string `json:"RoleArn,omitempty"`
}

func (EventStream) CloudJamType() string { return "AWS::Pinpoint::EventStream" }

type GCMChannel struct {
	ApiKey                      *string `json:"ApiKey,omitempty"`
	ApplicationId               *string `json:"ApplicationId,omitempty"`
	DefaultAuthenticationMethod *string `json:"DefaultAuthenticationMethod,omitempty"`
	Enabled                     *bool   `json:"Enabled,omitempty"`
	Id                          *string `json:"Id,omitempty"`
	ServiceJson                 *string `json:"ServiceJson,omitempty"`
}

func (GCMChannel) CloudJamType() string { return "AWS::Pinpoint::GCMChannel" }

type BodyConfig struct {
	Alignment *Alignment `json:"Alignment,omitempty"`
	Body      *string    `json:"Body,omitempty"`
	TextColor *string    `json:"TextColor,omitempty"`
}

type HeaderConfig struct {
	Alignment *Alignment `json:"Alignment,omitempty"`
	Header    *string    `json:"Header,omitempty"`
	TextColor *string    `json:"TextColor,omitempty"`
}

type InAppTemplateOverrideButtonConfiguration struct {
	ButtonAction *ButtonAction `json:"ButtonAction,omitempty"`
	Link         *string       `json:"Link,omitempty"`
}

type InAppTemplateDefaultButtonConfiguration struct {
	BackgroundColor *string       `json:"BackgroundColor,omitempty"`
	BorderRadius    *int          `json:"BorderRadius,omitempty"`
	ButtonAction    *ButtonAction `json:"ButtonAction,omitempty"`
	Link            *string       `json:"Link,omitempty"`
	Text            *string       `json:"Text,omitempty"`
	TextColor       *string       `json:"TextColor,omitempty"`
}

type ButtonConfig struct {
	Android       *InAppTemplateOverrideButtonConfiguration `json:"Android,omitempty"`
	DefaultConfig *InAppTemplateDefaultButtonConfiguration  `json:"DefaultConfig,omitempty"`
	IOS           *InAppTemplateOverrideButtonConfiguration `json:"IOS,omitempty"`
	Web           *InAppTemplateOverrideButtonConfiguration `json:"Web,omitempty"`
}

type InAppTemplateInAppMessageContent struct {
	BackgroundColor *string       `json:"BackgroundColor,omitempty"`
	BodyConfig      *BodyConfig   `json:"BodyConfig,omitempty"`
	HeaderConfig    *HeaderConfig `json:"HeaderConfig,omitempty"`
	ImageUrl        *string       `json:"ImageUrl,omitempty"`
	PrimaryBtn      *ButtonConfig `json:"PrimaryBtn,omitempty"`
	SecondaryBtn    *ButtonConfig `json:"SecondaryBtn,omitempty"`
}

type InAppTemplate struct {
	Arn                 *string                            `json:"Arn,omitempty"`
	Content             []InAppTemplateInAppMessageContent `json:"Content,omitempty"`
	CustomConfig        map[string]any                     `json:"CustomConfig,omitempty"`
	Layout              *InAppTemplateLayout               `json:"Layout,omitempty"`
	Tags                map[string]any                     `json:"Tags,omitempty"`
	TemplateDescription *string                            `json:"TemplateDescription,omitempty"`
	TemplateName        *string                            `json:"TemplateName,omitempty"`
}

func (InAppTemplate) CloudJamType() string { return "AWS::Pinpoint::InAppTemplate" }

type AndroidPushNotificationTemplate struct {
	Action            *string `json:"Action,omitempty"`
	Body              *string `json:"Body,omitempty"`
	ImageIconUrl      *string `json:"ImageIconUrl,omitempty"`
	ImageUrl          *string `json:"ImageUrl,omitempty"`
	SmallImageIconUrl *string `json:"SmallImageIconUrl,omitempty"`
	Sound             *string `json:"Sound,omitempty"`
	Title             *string `json:"Title,omitempty"`
	Url               *string `json:"Url,omitempty"`
}

type APNSPushNotificationTemplate struct {
	Action   *string `json:"Action,omitempty"`
	Body     *string `json:"Body,omitempty"`
	MediaUrl *string `json:"MediaUrl,omitempty"`
	Sound    *string `json:"Sound,omitempty"`
	Title    *string `json:"Title,omitempty"`
	Url      *string `json:"Url,omitempty"`
}

type DefaultPushNotificationTemplate struct {
	Action *string `json:"Action,omitempty"`
	Body   *string `json:"Body,omitempty"`
	Sound  *string `json:"Sound,omitempty"`
	Title  *string `json:"Title,omitempty"`
	Url    *string `json:"Url,omitempty"`
}

type PushTemplate struct {
	ADM                  *AndroidPushNotificationTemplate `json:"ADM,omitempty"`
	APNS                 *APNSPushNotificationTemplate    `json:"APNS,omitempty"`
	Arn                  *string                          `json:"Arn,omitempty"`
	Baidu                *AndroidPushNotificationTemplate `json:"Baidu,omitempty"`
	Default              *DefaultPushNotificationTemplate `json:"Default,omitempty"`
	DefaultSubstitutions *string                          `json:"DefaultSubstitutions,omitempty"`
	GCM                  *AndroidPushNotificationTemplate `json:"GCM,omitempty"`
	Id                   *string                          `json:"Id,omitempty"`
	Tags                 map[string]any                   `json:"Tags,omitempty"`
	TemplateDescription  *string                          `json:"TemplateDescription,omitempty"`
	TemplateName         *string                          `json:"TemplateName,omitempty"`
}

func (PushTemplate) CloudJamType() string { return "AWS::Pinpoint::PushTemplate" }

type SMSChannel struct {
	ApplicationId *string `json:"ApplicationId,omitempty"`
	Enabled       *bool   `json:"Enabled,omitempty"`
	Id            *string `json:"Id,omitempty"`
	SenderId      *string `json:"SenderId,omitempty"`
	ShortCode     *string `json:"ShortCode,omitempty"`
}

func (SMSChannel) CloudJamType() string { return "AWS::Pinpoint::SMSChannel" }

type Recency struct {
	Duration    *string `json:"Duration,omitempty"`
	RecencyType *string `json:"RecencyType,omitempty"`
}

type Behavior struct {
	Recency *Recency `json:"Recency,omitempty"`
}

type SegmentSetDimension struct {
	DimensionType *string  `json:"DimensionType,omitempty"`
	Values        []string `json:"Values,omitempty"`
}

type Demographic struct {
	AppVersion *SegmentSetDimension `json:"AppVersion,omitempty"`
	Channel    *SegmentSetDimension `json:"Channel,omitempty"`
	DeviceType *SegmentSetDimension `json:"DeviceType,omitempty"`
	Make       *SegmentSetDimension `json:"Make,omitempty"`
	Model      *SegmentSetDimension `json:"Model,omitempty"`
	Platform   *SegmentSetDimension `json:"Platform,omitempty"`
}

type Coordinates struct {
	Latitude  *float64 `json:"Latitude,omitempty"`
	Longitude *float64 `json:"Longitude,omitempty"`
}

type GPSPoint struct {
	Coordinates       *Coordinates `json:"Coordinates,omitempty"`
	RangeInKilometers *float64     `json:"RangeInKilometers,omitempty"`
}

type Location struct {
	Country  *SegmentSetDimension `json:"Country,omitempty"`
	GPSPoint *GPSPoint            `json:"GPSPoint,omitempty"`
}

type SegmentDimensions struct {
	Attributes     map[string]any `json:"Attributes,omitempty"`
	Behavior       *Behavior      `json:"Behavior,omitempty"`
	Demographic    *Demographic   `json:"Demographic,omitempty"`
	Location       *Location      `json:"Location,omitempty"`
	Metrics        map[string]any `json:"Metrics,omitempty"`
	UserAttributes map[string]any `json:"UserAttributes,omitempty"`
}

type SourceSegments struct {
	Id      *string `json:"Id,omitempty"`
	Version *int    `json:"Version,omitempty"`
}

type Groups struct {
	Dimensions     []SegmentDimensions `json:"Dimensions,omitempty"`
	SourceSegments []SourceSegments    `json:"SourceSegments,omitempty"`
	SourceType     *string             `json:"SourceType,omitempty"`
	Type           *string             `json:"Type,omitempty"`
}

type SegmentGroups struct {
	Groups  []Groups `json:"Groups,omitempty"`
	Include *string  `json:"Include,omitempty"`
}

type Segment struct {
	ApplicationId *string            `json:"ApplicationId,omitempty"`
	Arn           *string            `json:"Arn,omitempty"`
	Dimensions    *SegmentDimensions `json:"Dimensions,omitempty"`
	Name          *string            `json:"Name,omitempty"`
	SegmentGroups *SegmentGroups     `json:"SegmentGroups,omitempty"`
	SegmentId     *string            `json:"SegmentId,omitempty"`
	Tags          map[string]any     `json:"Tags,omitempty"`
}

func (Segment) CloudJamType() string { return "AWS::Pinpoint::Segment" }

type SmsTemplate struct {
	Arn                  *string        `json:"Arn,omitempty"`
	Body                 *string        `json:"Body,omitempty"`
	DefaultSubstitutions *string        `json:"DefaultSubstitutions,omitempty"`
	Id                   *string        `json:"Id,omitempty"`
	Tags                 map[string]any `json:"Tags,omitempty"`
	TemplateDescription  *string        `json:"TemplateDescription,omitempty"`
	TemplateName         *string        `json:"TemplateName,omitempty"`
}

func (SmsTemplate) CloudJamType() string { return "AWS::Pinpoint::SmsTemplate" }

type VoiceChannel struct {
	ApplicationId *string `json:"ApplicationId,omitempty"`
	Enabled       *bool   `json:"Enabled,omitempty"`
	Id            *string `json:"Id,omitempty"`
}

func (VoiceChannel) CloudJamType() string { return "AWS::Pinpoint::VoiceChannel" }

type Alignment string

const (
	AlignmentLEFT   Alignment = "LEFT"
	AlignmentCENTER Alignment = "CENTER"
	AlignmentRIGHT  Alignment = "RIGHT"
)

type ButtonAction string

const (
	ButtonActionLINK     ButtonAction = "LINK"
	ButtonActionDEEPLINK ButtonAction = "DEEP_LINK"
	ButtonActionCLOSE    ButtonAction = "CLOSE"
)

type InAppTemplateLayout string

const (
	InAppTemplateLayoutBOTTOMBANNER InAppTemplateLayout = "BOTTOM_BANNER"
	InAppTemplateLayoutTOPBANNER    InAppTemplateLayout = "TOP_BANNER"
	InAppTemplateLayoutOVERLAYS     InAppTemplateLayout = "OVERLAYS"
	InAppTemplateLayoutMOBILEFEED   InAppTemplateLayout = "MOBILE_FEED"
	InAppTemplateLayoutMIDDLEBANNER InAppTemplateLayout = "MIDDLE_BANNER"
	InAppTemplateLayoutCAROUSEL     InAppTemplateLayout = "CAROUSEL"
)
