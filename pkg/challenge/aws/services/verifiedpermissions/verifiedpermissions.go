// Code generated by go generate; DO NOT EDIT.

// Package verifiedpermissions holds the cloud control resources of the aws verifiedpermissions service.
package verifiedpermissions

import "encoding/json"

type IdentitySourceDetails struct {
	ClientIds    []string      `json:"ClientIds,omitempty"`
	DiscoveryUrl *string       `json:"DiscoveryUrl,omitempty"`
	OpenIdIssuer *OpenIdIssuer `json:"OpenIdIssuer,omitempty"`
	UserPoolArn  *string       `json:"UserPoolArn,omitempty"`
}

type IdentitySource struct {
	Configuration       json.RawMessage        `json:"Configuration,omitempty"`
	Details             *IdentitySourceDetails `json:"Details,omitempty"`
	IdentitySourceId    *string                `json:"IdentitySourceId,omitempty"`
	PolicyStoreId       *string                `json:"PolicyStoreId,omitempty"`
	PrincipalEntityType *string                `json:"PrincipalEntityType,omitempty"`
}

func (IdentitySource) CloudJamType() string { return "AWS::VerifiedPermissions::IdentitySource" }

type Policy struct {
	Definition    json.RawMessage `json:"Definition,omitempty"`
	Name          *string         `json:"Name,omitempty"`
	PolicyId      *string         `json:"PolicyId,omitempty"`
	PolicyStoreId *string         `json:"PolicyStoreId,omitempty"`
	PolicyType    *PolicyType     `json:"PolicyType,omitempty"`
}

func (Policy) CloudJamType() string { return "AWS::VerifiedPermissions::Policy" }

type DeletionProtection struct {
	Mode *DeletionMode `json:"Mode,omitempty"`
}

type SchemaDefinition struct {
	CedarJson *string `json:"CedarJson,omitempty"`
}

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type ValidationSettings struct {
	Mode *ValidationMode `json:"Mode,omitempty"`
}

type PolicyStore struct {
	Arn                *string             `json:"Arn,omitempty"`
	DeletionProtection *DeletionProtection `json:"DeletionProtection,omitempty"`
	Description        *string             `json:"Description,omitempty"`
	EncryptionSettings map[string]any      `json:"EncryptionSettings,omitempty"`
	EncryptionState    map[string]any      `json:"EncryptionState,omitempty"`
	PolicyStoreId      *string             `json:"PolicyStoreId,omitempty"`
	Schema             *SchemaDefinition   `json:"Schema,omitempty"`
	Tags               []Tag               `json:"Tags,omitempty"`
	ValidationSettings *ValidationSettings `json:"ValidationSettings,omitempty"`
}

func (PolicyStore) CloudJamType() string { return "AWS::VerifiedPermissions::PolicyStore" }

type PolicyStoreAlias struct {
	AliasName     *string `json:"AliasName,omitempty"`
	PolicyStoreId *string `json:"PolicyStoreId,omitempty"`
}

func (PolicyStoreAlias) CloudJamType() string { return "AWS::VerifiedPermissions::PolicyStoreAlias" }

type PolicyTemplate struct {
	Description      *string `json:"Description,omitempty"`
	Name             *string `json:"Name,omitempty"`
	PolicyStoreId    *string `json:"PolicyStoreId,omitempty"`
	PolicyTemplateId *string `json:"PolicyTemplateId,omitempty"`
	Statement        *string `json:"Statement,omitempty"`
}

func (PolicyTemplate) CloudJamType() string { return "AWS::VerifiedPermissions::PolicyTemplate" }

type OpenIdIssuer string

const (
	OpenIdIssuerCOGNITO OpenIdIssuer = "COGNITO"
)

type PolicyType string

const (
	PolicyTypeSTATIC         PolicyType = "STATIC"
	PolicyTypeTEMPLATELINKED PolicyType = "TEMPLATE_LINKED"
)

type DeletionMode string

const (
	DeletionModeENABLED  DeletionMode = "ENABLED"
	DeletionModeDISABLED DeletionMode = "DISABLED"
)

type ValidationMode string

const (
	ValidationModeOFF    ValidationMode = "OFF"
	ValidationModeSTRICT ValidationMode = "STRICT"
)
