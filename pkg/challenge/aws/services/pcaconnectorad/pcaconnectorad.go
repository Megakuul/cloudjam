// Code generated by go generate; DO NOT EDIT.

// Package pcaconnectorad holds the cloud control resources of the aws pcaconnectorad service.
package pcaconnectorad

import "encoding/json"

type VpcInformation struct {
	IpAddressType    *VpcInformationIpAddressType `json:"IpAddressType,omitempty"`
	SecurityGroupIds []string                     `json:"SecurityGroupIds,omitempty"`
}

type Connector struct {
	CertificateAuthorityArn *string           `json:"CertificateAuthorityArn,omitempty"`
	ConnectorArn            *string           `json:"ConnectorArn,omitempty"`
	DirectoryId             *string           `json:"DirectoryId,omitempty"`
	Tags                    map[string]string `json:"Tags,omitempty"`
	VpcInformation          *VpcInformation   `json:"VpcInformation,omitempty"`
}

func (Connector) CloudJamType() string { return "AWS::PCAConnectorAD::Connector" }

type DirectoryRegistration struct {
	DirectoryId              *string           `json:"DirectoryId,omitempty"`
	DirectoryRegistrationArn *string           `json:"DirectoryRegistrationArn,omitempty"`
	Tags                     map[string]string `json:"Tags,omitempty"`
}

func (DirectoryRegistration) CloudJamType() string {
	return "AWS::PCAConnectorAD::DirectoryRegistration"
}

type ServicePrincipalName struct {
	ConnectorArn             *string `json:"ConnectorArn,omitempty"`
	DirectoryRegistrationArn *string `json:"DirectoryRegistrationArn,omitempty"`
}

func (ServicePrincipalName) CloudJamType() string { return "AWS::PCAConnectorAD::ServicePrincipalName" }

type Template struct {
	ConnectorArn                  *string           `json:"ConnectorArn,omitempty"`
	Definition                    json.RawMessage   `json:"Definition,omitempty"`
	Name                          *string           `json:"Name,omitempty"`
	ReenrollAllCertificateHolders *bool             `json:"ReenrollAllCertificateHolders,omitempty"`
	Tags                          map[string]string `json:"Tags,omitempty"`
	TemplateArn                   *string           `json:"TemplateArn,omitempty"`
}

func (Template) CloudJamType() string { return "AWS::PCAConnectorAD::Template" }

type AccessRights struct {
	AutoEnroll *AccessRight `json:"AutoEnroll,omitempty"`
	Enroll     *AccessRight `json:"Enroll,omitempty"`
}

type TemplateGroupAccessControlEntry struct {
	AccessRights            *AccessRights `json:"AccessRights,omitempty"`
	GroupDisplayName        *string       `json:"GroupDisplayName,omitempty"`
	GroupSecurityIdentifier *string       `json:"GroupSecurityIdentifier,omitempty"`
	TemplateArn             *string       `json:"TemplateArn,omitempty"`
}

func (TemplateGroupAccessControlEntry) CloudJamType() string {
	return "AWS::PCAConnectorAD::TemplateGroupAccessControlEntry"
}

type VpcInformationIpAddressType string

const (
	VpcInformationIpAddressTypeIPV4      VpcInformationIpAddressType = "IPV4"
	VpcInformationIpAddressTypeDUALSTACK VpcInformationIpAddressType = "DUALSTACK"
)

type AccessRight string

const (
	AccessRightALLOW AccessRight = "ALLOW"
	AccessRightDENY  AccessRight = "DENY"
)
