// Code generated by go generate; DO NOT EDIT.

// Package sqs holds the cloud control resources of the aws sqs service.
package sqs

import "encoding/json"

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Queue struct {
	Arn                           *string         `json:"Arn,omitempty"`
	ContentBasedDeduplication     *bool           `json:"ContentBasedDeduplication,omitempty"`
	DeduplicationScope            *string         `json:"DeduplicationScope,omitempty"`
	DelaySeconds                  *int            `json:"DelaySeconds,omitempty"`
	FifoQueue                     *bool           `json:"FifoQueue,omitempty"`
	FifoThroughputLimit           *string         `json:"FifoThroughputLimit,omitempty"`
	KmsDataKeyReusePeriodSeconds  *int            `json:"KmsDataKeyReusePeriodSeconds,omitempty"`
	KmsMasterKeyId                *string         `json:"KmsMasterKeyId,omitempty"`
	MaximumMessageSize            *int            `json:"MaximumMessageSize,omitempty"`
	MessageRetentionPeriod        *int            `json:"MessageRetentionPeriod,omitempty"`
	QueueName                     *string         `json:"QueueName,omitempty"`
	QueueUrl                      *string         `json:"QueueUrl,omitempty"`
	ReceiveMessageWaitTimeSeconds *int            `json:"ReceiveMessageWaitTimeSeconds,omitempty"`
	RedriveAllowPolicy            json.RawMessage `json:"RedriveAllowPolicy,omitempty"`
	RedrivePolicy                 json.RawMessage `json:"RedrivePolicy,omitempty"`
	SqsManagedSseEnabled          *bool           `json:"SqsManagedSseEnabled,omitempty"`
	Tags                          []Tag           `json:"Tags,omitempty"`
	VisibilityTimeout             *int            `json:"VisibilityTimeout,omitempty"`
}

func (Queue) CloudJamType() string { return "AWS::SQS::Queue" }

type QueueInlinePolicy struct {
	PolicyDocument map[string]any `json:"PolicyDocument,omitempty"`
	Queue          *string        `json:"Queue,omitempty"`
}

func (QueueInlinePolicy) CloudJamType() string { return "AWS::SQS::QueueInlinePolicy" }

type QueuePolicy struct {
	Id             *string         `json:"Id,omitempty"`
	PolicyDocument json.RawMessage `json:"PolicyDocument,omitempty"`
	Queues         []string        `json:"Queues,omitempty"`
}

func (QueuePolicy) CloudJamType() string { return "AWS::SQS::QueuePolicy" }
