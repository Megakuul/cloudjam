<script lang="ts">
	...
	// listing games scans the table, so only a page worth of them is pulled at a time.
	const limit = 20;
		...
		<Table.Root>
			...
			<Table.Body>
				{#each games as game (game.id)}
					<Table.Row class="cursor-pointer" onclick={() => open(game.id)}>
