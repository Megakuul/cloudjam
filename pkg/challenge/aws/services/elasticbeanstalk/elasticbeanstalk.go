// Code generated by go generate; DO NOT EDIT.

// Package elasticbeanstalk holds the cloud control resources of the aws elasticbeanstalk service.
package elasticbeanstalk

type MaxAgeRule struct {
	DeleteSourceFromS3 *bool `json:"DeleteSourceFromS3,omitempty"`
	Enabled            *bool `json:"Enabled,omitempty"`
	MaxAgeInDays       *int  `json:"MaxAgeInDays,omitempty"`
}

type MaxCountRule struct {
	DeleteSourceFromS3 *bool `json:"DeleteSourceFromS3,omitempty"`
	Enabled            *bool `json:"Enabled,omitempty"`
	MaxCount           *int  `json:"MaxCount,omitempty"`
}

type ApplicationVersionLifecycleConfig struct {
	MaxAgeRule   *MaxAgeRule   `json:"MaxAgeRule,omitempty"`
	MaxCountRule *MaxCountRule `json:"MaxCountRule,omitempty"`
}

type ApplicationResourceLifecycleConfig struct {
	ServiceRole            *string                            `json:"ServiceRole,omitempty"`
	VersionLifecycleConfig *ApplicationVersionLifecycleConfig `json:"VersionLifecycleConfig,omitempty"`
}

type Application struct {
	ApplicationName         *string                             `json:"ApplicationName,omitempty"`
	Description             *string                             `json:"Description,omitempty"`
	ResourceLifecycleConfig *ApplicationResourceLifecycleConfig `json:"ResourceLifecycleConfig,omitempty"`
}

func (Application) CloudJamType() string { return "AWS::ElasticBeanstalk::Application" }

type SourceBundle struct {
	S3Bucket *string `json:"S3Bucket,omitempty"`
	S3Key    *string `json:"S3Key,omitempty"`
}

type ApplicationVersion struct {
	ApplicationName *string       `json:"ApplicationName,omitempty"`
	Description     *string       `json:"Description,omitempty"`
	Id              *string       `json:"Id,omitempty"`
	SourceBundle    *SourceBundle `json:"SourceBundle,omitempty"`
}

func (ApplicationVersion) CloudJamType() string { return "AWS::ElasticBeanstalk::ApplicationVersion" }

type ConfigurationOptionSetting struct {
	Namespace    *string `json:"Namespace,omitempty"`
	OptionName   *string `json:"OptionName,omitempty"`
	ResourceName *string `json:"ResourceName,omitempty"`
	Value        *string `json:"Value,omitempty"`
}

type SourceConfiguration struct {
	ApplicationName *string `json:"ApplicationName,omitempty"`
	TemplateName    *string `json:"TemplateName,omitempty"`
}

type ConfigurationTemplate struct {
	ApplicationName     *string                      `json:"ApplicationName,omitempty"`
	Description         *string                      `json:"Description,omitempty"`
	EnvironmentId       *string                      `json:"EnvironmentId,omitempty"`
	OptionSettings      []ConfigurationOptionSetting `json:"OptionSettings,omitempty"`
	PlatformArn         *string                      `json:"PlatformArn,omitempty"`
	SolutionStackName   *string                      `json:"SolutionStackName,omitempty"`
	SourceConfiguration *SourceConfiguration         `json:"SourceConfiguration,omitempty"`
	TemplateName        *string                      `json:"TemplateName,omitempty"`
}

func (ConfigurationTemplate) CloudJamType() string {
	return "AWS::ElasticBeanstalk::ConfigurationTemplate"
}

type OptionSetting struct {
	Namespace    *string `json:"Namespace,omitempty"`
	OptionName   *string `json:"OptionName,omitempty"`
	ResourceName *string `json:"ResourceName,omitempty"`
	Value        *string `json:"Value,omitempty"`
}

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Tier struct {
	Name    *string `json:"Name,omitempty"`
	Type    *string `json:"Type,omitempty"`
	Version *string `json:"Version,omitempty"`
}

type Environment struct {
	ApplicationName   *string         `json:"ApplicationName,omitempty"`
	CNAMEPrefix       *string         `json:"CNAMEPrefix,omitempty"`
	Description       *string         `json:"Description,omitempty"`
	EndpointURL       *string         `json:"EndpointURL,omitempty"`
	EnvironmentName   *string         `json:"EnvironmentName,omitempty"`
	OperationsRole    *string         `json:"OperationsRole,omitempty"`
	OptionSettings    []OptionSetting `json:"OptionSettings,omitempty"`
	PlatformArn       *string         `json:"PlatformArn,omitempty"`
	SolutionStackName *string         `json:"SolutionStackName,omitempty"`
	Tags              []Tag           `json:"Tags,omitempty"`
	TemplateName      *string         `json:"TemplateName,omitempty"`
	Tier              *Tier           `json:"Tier,omitempty"`
	VersionLabel      *string         `json:"VersionLabel,omitempty"`
}

func (Environment) CloudJamType() string { return "AWS::ElasticBeanstalk::Environment" }
