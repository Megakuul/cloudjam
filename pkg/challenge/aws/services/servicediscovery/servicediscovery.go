// Code generated by go generate; DO NOT EDIT.

// Package servicediscovery holds the cloud control resources of the aws servicediscovery service.
package servicediscovery

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type HttpNamespace struct {
	Arn         *string `json:"Arn,omitempty"`
	Description *string `json:"Description,omitempty"`
	Id          *string `json:"Id,omitempty"`
	Name        *string `json:"Name,omitempty"`
	Tags        []Tag   `json:"Tags,omitempty"`
}

func (HttpNamespace) CloudJamType() string { return "AWS::ServiceDiscovery::HttpNamespace" }

type Instance struct {
	InstanceAttributes map[string]any `json:"InstanceAttributes,omitempty"`
	InstanceId         *string        `json:"InstanceId,omitempty"`
	ServiceId          *string        `json:"ServiceId,omitempty"`
}

func (Instance) CloudJamType() string { return "AWS::ServiceDiscovery::Instance" }

type SOA struct {
	TTL *float64 `json:"TTL,omitempty"`
}

type PrivateDnsPropertiesMutable struct {
	SOA *SOA `json:"SOA,omitempty"`
}

type Properties struct {
	DnsProperties *PrivateDnsPropertiesMutable `json:"DnsProperties,omitempty"`
}

type PrivateDnsNamespaceTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type PrivateDnsNamespace struct {
	Arn          *string                  `json:"Arn,omitempty"`
	Description  *string                  `json:"Description,omitempty"`
	HostedZoneId *string                  `json:"HostedZoneId,omitempty"`
	Id           *string                  `json:"Id,omitempty"`
	Name         *string                  `json:"Name,omitempty"`
	Properties   *Properties              `json:"Properties,omitempty"`
	Tags         []PrivateDnsNamespaceTag `json:"Tags,omitempty"`
	Vpc          *string                  `json:"Vpc,omitempty"`
}

func (PrivateDnsNamespace) CloudJamType() string { return "AWS::ServiceDiscovery::PrivateDnsNamespace" }

type PublicDnsNamespaceSOA struct {
	TTL *float64 `json:"TTL,omitempty"`
}

type PublicDnsPropertiesMutable struct {
	SOA *PublicDnsNamespaceSOA `json:"SOA,omitempty"`
}

type PublicDnsNamespaceProperties struct {
	DnsProperties *PublicDnsPropertiesMutable `json:"DnsProperties,omitempty"`
}

type PublicDnsNamespaceTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type PublicDnsNamespace struct {
	Arn          *string                       `json:"Arn,omitempty"`
	Description  *string                       `json:"Description,omitempty"`
	HostedZoneId *string                       `json:"HostedZoneId,omitempty"`
	Id           *string                       `json:"Id,omitempty"`
	Name         *string                       `json:"Name,omitempty"`
	Properties   *PublicDnsNamespaceProperties `json:"Properties,omitempty"`
	Tags         []PublicDnsNamespaceTag       `json:"Tags,omitempty"`
}

func (PublicDnsNamespace) CloudJamType() string { return "AWS::ServiceDiscovery::PublicDnsNamespace" }

type DnsRecord struct {
	TTL  *float64 `json:"TTL,omitempty"`
	Type *string  `json:"Type,omitempty"`
}

type DnsConfig struct {
	DnsRecords    []DnsRecord `json:"DnsRecords,omitempty"`
	NamespaceId   *string     `json:"NamespaceId,omitempty"`
	RoutingPolicy *string     `json:"RoutingPolicy,omitempty"`
}

type HealthCheckConfig struct {
	FailureThreshold *float64 `json:"FailureThreshold,omitempty"`
	ResourcePath     *string  `json:"ResourcePath,omitempty"`
	Type             *string  `json:"Type,omitempty"`
}

type HealthCheckCustomConfig struct {
	FailureThreshold *float64 `json:"FailureThreshold,omitempty"`
}

type ServiceTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type Service struct {
	Arn                     *string                  `json:"Arn,omitempty"`
	Description             *string                  `json:"Description,omitempty"`
	DnsConfig               *DnsConfig               `json:"DnsConfig,omitempty"`
	HealthCheckConfig       *HealthCheckConfig       `json:"HealthCheckConfig,omitempty"`
	HealthCheckCustomConfig *HealthCheckCustomConfig `json:"HealthCheckCustomConfig,omitempty"`
	Id                      *string                  `json:"Id,omitempty"`
	Name                    *string                  `json:"Name,omitempty"`
	NamespaceId             *string                  `json:"NamespaceId,omitempty"`
	ServiceAttributes       map[string]string        `json:"ServiceAttributes,omitempty"`
	Tags                    []ServiceTag             `json:"Tags,omitempty"`
	Type                    *string                  `json:"Type,omitempty"`
}

func (Service) CloudJamType() string { return "AWS::ServiceDiscovery::Service" }
