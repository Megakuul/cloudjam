// Code generated by go generate; DO NOT EDIT.

// Package inspector holds the cloud control resources of the aws inspector service.
package inspector

type AssessmentTarget struct {
	Arn                  *string `json:"Arn,omitempty"`
	AssessmentTargetName *string `json:"AssessmentTargetName,omitempty"`
	ResourceGroupArn     *string `json:"ResourceGroupArn,omitempty"`
}

func (AssessmentTarget) CloudJamType() string { return "AWS::Inspector::AssessmentTarget" }

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type AssessmentTemplate struct {
	Arn                       *string  `json:"Arn,omitempty"`
	AssessmentTargetArn       *string  `json:"AssessmentTargetArn,omitempty"`
	AssessmentTemplateName    *string  `json:"AssessmentTemplateName,omitempty"`
	DurationInSeconds         *int     `json:"DurationInSeconds,omitempty"`
	RulesPackageArns          []string `json:"RulesPackageArns,omitempty"`
	UserAttributesForFindings []Tag    `json:"UserAttributesForFindings,omitempty"`
}

func (AssessmentTemplate) CloudJamType() string { return "AWS::Inspector::AssessmentTemplate" }

type ResourceGroupTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type ResourceGroup struct {
	Arn               *string            `json:"Arn,omitempty"`
	ResourceGroupTags []ResourceGroupTag `json:"ResourceGroupTags,omitempty"`
}

func (ResourceGroup) CloudJamType() string { return "AWS::Inspector::ResourceGroup" }
