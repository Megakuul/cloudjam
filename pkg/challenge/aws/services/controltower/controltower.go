// Code generated by go generate; DO NOT EDIT.

// Package controltower holds the cloud control resources of the aws controltower service.
package controltower

import "encoding/json"

type Parameter struct {
	Key   *string         `json:"Key,omitempty"`
	Value json.RawMessage `json:"Value,omitempty"`
}

type Tag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type EnabledBaseline struct {
	BaselineIdentifier        *string     `json:"BaselineIdentifier,omitempty"`
	BaselineVersion           *string     `json:"BaselineVersion,omitempty"`
	EnabledBaselineIdentifier *string     `json:"EnabledBaselineIdentifier,omitempty"`
	Parameters                []Parameter `json:"Parameters,omitempty"`
	Tags                      []Tag       `json:"Tags,omitempty"`
	TargetIdentifier          *string     `json:"TargetIdentifier,omitempty"`
}

func (EnabledBaseline) CloudJamType() string { return "AWS::ControlTower::EnabledBaseline" }

type EnabledControlParameter struct {
	Key   *string         `json:"Key,omitempty"`
	Value json.RawMessage `json:"Value,omitempty"`
}

type EnabledControlTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type EnabledControl struct {
	ControlIdentifier *string                   `json:"ControlIdentifier,omitempty"`
	Parameters        []EnabledControlParameter `json:"Parameters,omitempty"`
	Tags              []EnabledControlTag       `json:"Tags,omitempty"`
	TargetIdentifier  *string                   `json:"TargetIdentifier,omitempty"`
}

func (EnabledControl) CloudJamType() string { return "AWS::ControlTower::EnabledControl" }

type LandingZoneTag struct {
	Key   *string `json:"Key,omitempty"`
	Value *string `json:"Value,omitempty"`
}

type LandingZone struct {
	Arn                    *string                 `json:"Arn,omitempty"`
	DriftStatus            *LandingZoneDriftStatus `json:"DriftStatus,omitempty"`
	LandingZoneIdentifier  *string                 `json:"LandingZoneIdentifier,omitempty"`
	LatestAvailableVersion *string                 `json:"LatestAvailableVersion,omitempty"`
	Manifest               json.RawMessage         `json:"Manifest,omitempty"`
	RemediationTypes       []RemediationTypesItem  `json:"RemediationTypes,omitempty"`
	Status                 *LandingZoneStatus      `json:"Status,omitempty"`
	Tags                   []LandingZoneTag        `json:"Tags,omitempty"`
	Version                *string                 `json:"Version,omitempty"`
}

func (LandingZone) CloudJamType() string { return "AWS::ControlTower::LandingZone" }

type LandingZoneDriftStatus string

const (
	LandingZoneDriftStatusDRIFTED LandingZoneDriftStatus = "DRIFTED"
	LandingZoneDriftStatusINSYNC  LandingZoneDriftStatus = "IN_SYNC"
)

type RemediationTypesItem string

const (
	RemediationTypesItemINHERITANCEDRIFT RemediationTypesItem = "INHERITANCE_DRIFT"
)

type LandingZoneStatus string

const (
	LandingZoneStatusACTIVE     LandingZoneStatus = "ACTIVE"
	LandingZoneStatusPROCESSING LandingZoneStatus = "PROCESSING"
	LandingZoneStatusFAILED     LandingZoneStatus = "FAILED"
)
